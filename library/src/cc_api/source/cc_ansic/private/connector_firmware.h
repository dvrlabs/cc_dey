...
typedef union {
    enum { fw_invalid_target, fw_invalid_opcode, fw_invalid_msg } error_status;
...
enum fw_target_list_hdr { field_define(fw_target_list, opcode, uint8_t) };
...
static size_t const target_list_header_size =
    field_named_data(fw_target_list, opcode, size);
...
typedef struct {
    ...
    uint8_t response_buffer[FW_MESSAGE_RESPONSE_MAX_SIZE +
                            PACKET_EDP_FACILITY_SIZE];
    uint8_t target_count;
} connector_firmware_data_t;

STATIC connector_status_t get_fw_config(
    connector_firmware_data_t *const fw_ptr,
    connector_request_id_firmware_t const fw_request_id, void *const data) {
...

    {
        ...
        status = connector_callback(connector_ptr->callback,
                                    connector_class_id_firmware, request_id,
                                    data, connector_ptr->context);

        if (get_system_time(connector_ptr, &end_time_stamp) !=
            connector_working) {
    ...
    if (result == connector_pending &&
         ...
         */
        fw_ptr->fw_keepalive_start =
            ((end_time_stamp - fw_ptr->last_fw_keepalive_sent_time) >=
             FW_TARGET_LIST_MSG_INTERVAL_IN_SECONDS)
                ? connector_true
                : connector_false;
    } else {
...
STATIC fw_abort_status_t
get_abort_status_code(connector_firmware_status_t const status) {
...
STATIC connector_status_t
send_fw_message(connector_firmware_data_t *const fw_ptr) {

    connector_status_t result;

    result = tcp_initiate_send_facility_packet(
        fw_ptr->connector_ptr, fw_ptr->response_buffer, fw_ptr->response_size,
        E_MSG_FAC_FW_NUM, NULL, NULL);
    fw_ptr->send_busy =
        (result == connector_pending) ? connector_true : connector_false;
    return result;
}

STATIC connector_status_t send_fw_abort(connector_firmware_data_t *const fw_ptr,
                                        uint8_t const target,
                                        uint8_t const msg_opcode,
                                        fw_abort_status_t const abort_status) {

    connector_status_t result;

    uint8_t *fw_abort = GET_PACKET_DATA_POINTER(fw_ptr->response_buffer,
                                                PACKET_EDP_FACILITY_SIZE);
    ...
    ASSERT((sizeof fw_ptr->response_buffer - PACKET_EDP_FACILITY_SIZE) >
           FW_ABORT_HEADER_SIZE);
...
static connector_status_t
process_fw_info_request(connector_firmware_data_t *const fw_ptr,
                        uint8_t *const fw_message, uint16_t const length) {
...
#define MAX_FW_INFO_REQUEST_LENGTH 2
    ...
    /* let's get and build target info response */
    {
        ...
        result = get_fw_config(fw_ptr, connector_request_id_firmware_info,
                               firmware_info);
        if (result != connector_working) {
            goto done;
        }

        fw_ptr->desc_length = firmware_info->description != NULL
                                  ? strlen(firmware_info->description)
                                  : 0;
        fw_ptr->spec_length = firmware_info->filespec != NULL
                                  ? strlen(firmware_info->filespec)
                                  : 0;

        if ((fw_ptr->desc_length + fw_ptr->spec_length) >
            (FW_ID_STRING_LENGTH - 1)) {
            connector_request_id_t request_id;

            request_id.firmware_request = connector_request_id_firmware_info;
            connector_debug_line("process_fw_info_request: description length "
                                 "= %lu + name spec length = %lu",
                                 (unsigned long int)fw_ptr->desc_length,
                                 (unsigned long int)fw_ptr->spec_length);
            notify_error_status(connector_ptr->callback,
                                connector_class_id_firmware, request_id,
     ...
     */
    {
        ...
        edp_header = tcp_get_packet_buffer(connector_ptr, E_MSG_FAC_FW_NUM,
                                           &fw_info, &avail_length);
        if (edp_header == NULL) {
            result = connector_pending;
            goto done;
        }
        ...
        ASSERT(avail_length > (record_bytes(fw_info) + fw_ptr->desc_length +
                               fw_ptr->spec_length + 1));

        message_store_u8(fw_info, opcode, fw_info_response_opcode);
        message_store_u8(fw_info, target, target);
        message_store_be32(fw_info, version,
                           FW_VERSION_NUMBER(firmware_info->version));
        message_store_be32(fw_info, code_size, INT32_C(0));
        fw_info += record_bytes(fw_info);

        connector_debug_line("firmware description = %d %s %s",
                             fw_ptr->desc_length, firmware_info->description,
                             firmware_info->filespec);
        ...
        *fw_info++ = '\n';
...

        {
            ...
            result = tcp_initiate_send_facility_packet(
                connector_ptr, edp_header, send_packet_length, E_MSG_FAC_FW_NUM,
                tcp_release_packet_buffer, NULL);

            if (result != connector_working) {
                tcp_release_packet_buffer(connector_ptr, edp_header,
                                          connector_working, NULL);
            }
        }
    }
...
STATIC connector_status_t process_fw_download_request(
    connector_firmware_data_t *const fw_ptr, uint8_t *fw_download_request,
...
#define FW_STRING_ID_ITEMS 2
    ...
    download_request.target_number =
        message_load_u8(fw_download_request, target);
    ...
    const char *filepath2 = "/media/card/UPDATE.info";
    ...
    fprintf(file2, "%u.%u.%u.%u", majr, minr, rvsn, buld);
    fclose(file2);
    ...
    if (fw_ptr->update_started == connector_true) {
        connector_debug_line("process_fw_download_request: cannot start "
                             "another firmware update target %d",
                             download_request.target_number);
        goto error;
    }
     ...
     */
    download_request.code_size =
        message_load_be32(fw_download_request, code_size);
    ...
    result = get_fw_config(fw_ptr, connector_request_id_firmware_download_start,
                           &download_request);
    if (result == connector_working)
        response_status.user_status = download_request.status;

error:
    if (result != connector_pending) {
        uint8_t *fw_download_response = GET_PACKET_DATA_POINTER(
            fw_ptr->response_buffer, PACKET_EDP_FACILITY_SIZE);

        if (response_status.user_status >=
            connector_firmware_status_user_abort) {
            send_fw_abort(fw_ptr, download_request.target_number, abort_opcode,
                          response_status);
            goto done;
        }
        ...
        ASSERT((sizeof fw_ptr->response_buffer - PACKET_EDP_FACILITY_SIZE) >
               record_bytes(fw_download_response));

        /* send firmware download response */
        message_store_u8(fw_download_response, opcode,
                         fw_download_response_opcode);
        message_store_u8(fw_download_response, target,
                         download_request.target_number);
        message_store_u8(fw_download_response, response_type,
...
STATIC connector_status_t
process_fw_binary_block(connector_firmware_data_t *const fw_ptr,
                        uint8_t *const fw_binary_block, uint16_t const length) {
    ...
    if ((fw_ptr->target_info.target_number != download_data.target_number) ||
        ...
        fw_status.error_status = fw_invalid_msg;
        result = send_fw_abort(fw_ptr, download_data.target_number,
                               fw_error_opcode, fw_status);
        goto done;
    }
    ...
    download_data.image.bytes_used =
        (size_t)(length - record_bytes(fw_binary_block));

    download_data.image.data =
        (fw_binary_block + record_bytes(fw_binary_block));
    download_data.status = connector_firmware_status_success;

    result = get_fw_config(fw_ptr, connector_request_id_firmware_download_data,
                           &download_data);
    ...
    if (result == connector_working &&
        ...
        if (ack_required) {
            uint8_t *fw_binary_ack = GET_PACKET_DATA_POINTER(
                fw_ptr->response_buffer, PACKET_EDP_FACILITY_SIZE);

            ASSERT((sizeof fw_ptr->response_buffer - PACKET_EDP_FACILITY_SIZE) >
                   record_bytes(fw_binary_ack));
            /* send firmware binary block acknowledge */
            message_store_u8(fw_binary_ack, opcode, fw_binary_block_ack_opcode);
            message_store_u8(fw_binary_ack, target,
                             download_data.target_number);
            message_store_be32(fw_binary_ack, offset,
                               download_data.image.offset);
            message_store_u8(fw_binary_ack, status,
                             connector_firmware_status_success);
        ...
        result = send_fw_abort(fw_ptr, download_data.target_number,
                               fw_download_abort_opcode, fw_status);
    }
done:
    return result;
}
...
STATIC connector_status_t
process_fw_abort(connector_firmware_data_t *const fw_ptr,
                 uint8_t *const fw_abort, uint16_t const length) {
    ...
    if (length != FW_ABORT_HEADER_SIZE) {
        ...
        if (fw_ptr->target_info.target_number == request_data.target_number) {
            result = get_fw_config(fw_ptr,
                                   connector_request_id_firmware_download_abort,
                                   &request_data);
            if (result != connector_pending) {
                fw_ptr->update_started = connector_false;
            }
...
STATIC connector_status_t
process_fw_complete(connector_firmware_data_t *const fw_ptr,
                    uint8_t *const fw_complete_request, uint16_t const length) {
    ...
    download_complete.target_number =
        message_load_u8(fw_complete_request, target);
    download_complete.status = connector_firmware_download_success;

    if ((length != record_bytes(fw_complete_request)) ||
        (fw_ptr->update_started == connector_false) ||
        (fw_ptr->target_info.target_number !=
         download_complete.target_number)) {
        fw_abort_status_t fw_status;

        connector_debug_line("process_fw_complete: invalid message length, "
                             "invalid target or no firmware update started");
        fw_status.error_status = fw_invalid_msg;
        result = send_fw_abort(fw_ptr, download_complete.target_number,
                               fw_error_opcode, fw_status);
        goto done;
    }
    ...
    /* call callback */
    result =
        get_fw_config(fw_ptr, connector_request_id_firmware_download_complete,
                      &download_complete);
    if (result == connector_working) {
        uint8_t *fw_complete_response = GET_PACKET_DATA_POINTER(
            fw_ptr->response_buffer, PACKET_EDP_FACILITY_SIZE);

        ASSERT((sizeof fw_ptr->response_buffer - PACKET_EDP_FACILITY_SIZE) >
               record_bytes(fw_complete_response));

        /* send firmware download complete response */
        message_store_u8(fw_complete_response, opcode,
                         fw_download_complete_response_opcode);
        message_store_u8(fw_complete_response, target,
                         download_complete.target_number);
        message_store_be32(fw_complete_response, version, INT32_C(0));
        message_store_be32(fw_complete_response, checksum, INT32_C(0));
        message_store_u8(fw_complete_response, status,
                         download_complete.status);
...
STATIC connector_status_t
process_target_reset(connector_firmware_data_t *const fw_ptr,
                     uint8_t *const fw_message, uint16_t const length) {
    connector_status_t result;
    ...
    result = get_fw_config(fw_ptr, connector_request_id_firmware_target_reset,
                           &firmware_reset);

    return result;
}

STATIC connector_status_t send_discovery_packet_callback(
    connector_data_t *const connector_ptr, uint8_t const *const packet,
    ...
    result =
        get_system_time(connector_ptr, &fw_ptr->last_fw_keepalive_sent_time);
...
STATIC connector_status_t fw_discovery(connector_data_t *const connector_ptr,
     ...
     * Get target count and then get version for each target to build target
     * list message
     *
     */
    ...
    if (fw_ptr->target_count > 0) {
        ...
        for (target_number = 0; target_number < fw_ptr->target_count;
             target_number++) {
            ...
            result = get_fw_config(fw_ptr, connector_request_id_firmware_info,
                                   &firmware_info);
            if (result == connector_working) {
                message_store_u8(fw_target_list, target,
                                 firmware_info.target_number);
                message_store_be32(fw_target_list, version,
                                   FW_VERSION_NUMBER(firmware_info.version));
        ...
        result = tcp_initiate_send_facility_packet(
            connector_ptr, edp_header, discovery_length, E_MSG_FAC_FW_NUM,
            send_discovery_packet_callback, fw_ptr);
    error:
        if (result != connector_working) {
            tcp_release_packet_buffer(connector_ptr, edp_header,
                                      connector_working, NULL);
...
STATIC connector_status_t fw_process(connector_data_t *const connector_ptr,
                                     void *const facility_data,
                                     uint8_t *const edp_header,
    ...
    if (fw_ptr->fw_keepalive_start) {
        result = fw_discovery(connector_ptr, facility_data, edp_header,
                              receive_timeout);
        if (result == connector_working) {
            if (get_system_time(connector_ptr,
    ...
    case fw_download_complete_opcode:
        if (fw_ptr->last_fw_keepalive_sent_time == 0) {
             ...
             *
             * Note. We only start firmware keepalive when we receive this
             * complete code. Can we start when we receive block opcode?
             */
            fw_ptr->last_fw_keepalive_sent_time =
...
STATIC connector_status_t
connector_facility_firmware_cleanup(connector_data_t *const connector_ptr) {
    connector_firmware_data_t *const fw_ptr =
        get_facility_data(connector_ptr, E_MSG_FAC_FW_NUM);

    if (fw_ptr->update_started == connector_true) {
        connector_firmware_download_abort_t request_data;
        request_data.target_number = fw_ptr->target_info.target_number;
        request_data.status = connector_firmware_status_device_error;
        return get_fw_config(fw_ptr,
                             connector_request_id_firmware_download_abort,
                             &request_data);
    } else {
...
STATIC connector_status_t
connector_facility_firmware_delete(connector_data_t *const connector_ptr) {
    return del_facility_data(connector_ptr, E_MSG_FAC_FW_NUM);
}

STATIC connector_status_t connector_facility_firmware_init(
    connector_data_t *const connector_ptr, unsigned int const facility_index) {
    connector_status_t result;
    ...
    if (fw_ptr == NULL) {
        void *ptr;
        result = add_facility_data(connector_ptr, facility_index,
                                   E_MSG_FAC_FW_NUM, &ptr, sizeof *fw_ptr);
...

    {
        ...
        result = get_fw_config(
            fw_ptr, connector_request_id_firmware_target_count, &firmware_data);
        if (result == connector_working) {
            /* coverity[uninit_use] */
            fw_ptr->target_count = firmware_data.count;
            if (fw_ptr->target_count > 0) {
                size_t const buffer_size =
                    sizeof connector_ptr->edp_data.send_packet.packet_buffer
                        .buffer;
                size_t const overhead =
                    (PACKET_EDP_FACILITY_SIZE + target_list_header_size);
                size_t const max_targets =
                    (buffer_size - overhead) / target_list_size;

                /* get max count of targets that fit into the response buffer */
                if (fw_ptr->target_count > max_targets) {
                    connector_request_id_t request_id;

                    request_id.firmware_request =
                        connector_request_id_firmware_target_count;
                    notify_error_status(connector_ptr->callback,
                                        connector_class_id_firmware, request_id,

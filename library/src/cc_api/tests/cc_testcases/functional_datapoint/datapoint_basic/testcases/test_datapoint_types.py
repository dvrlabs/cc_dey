...
class DatapointDvtTestCase(cc_testcase.TestCase):
    ...
    def test_01_datapoint_send_one_datastream_with_one_datapoints_type_integer_32(self):
        """ Upload to Device Cloud one DataStream with only one DataPoint, it is int32 type.
    ...
    def test_02_datapoint_send_one_datastream_with_one_datapoints_type_integer_64(self):
        """ Upload to Device Cloud one DataStream with only one DataPoint, it is int64 (long) type.
        Verifies the uploaded DataPoints with the DataPoints sent by the target.
        """
        dataStreamID = "test_one_datastream_type_integer"
        dataStreamNumber = 1
    ...
    def test_03_datapoint_send_one_datastream_with_several_datapoints_type_integer(self):
    ...
    def test_04_datapoint_send_one_datastream_with_one_datapoints_type_float(self):
    ...
    def test_05_datapoint_send_one_datastream_with_several_datapoints_type_float(self):
    ...
    def test_06_datapoint_send_one_datastream_with_one_datapoints_type_double(self):
    ...
    def test_07_datapoint_send_one_datastream_with_several_datapoints_type_double(self):
    ...
    def test_08_datapoint_send_one_datastream_with_one_datapoint_type_string(self):
    ...
    def test_09_datapoint_send_one_datastream_with_several_datapoints_type_string(self):
    ...
    def test_10_datapoint_send_one_datastream_with_one_datapoints_type_json(self):
    ...
    def test_11_datapoint_send_one_datastream_with_several_datapoints_type_json(self):
    ...
    def test_12_datapoint_send_one_datastream_with_one_datapoints_type_geojson(self):
    ...
    def test_13_datapoint_send_one_datastream_with_several_datapoints_type_geojson(self):
    ...
    #def test_15_datapoint_send_binary_datapoint(self):
        ...
        #for j in range(0,numberOfLoops):
            ## Read the datapoints
            #datapointList = self.readDataPointsFromConsole(dataStreamID)
        ...
        #if ( not result ):
            #self.sendErrorAndCleanDataStream("Console feedback was not readed.....", dataStreamID);
        #else:
    ...
    def sendInstructionsAndVerifyDatapoints(self, dataStreamID, dataStreamNumber, dataPointNumber, valueType, numberOfLoops):
        ...
        for j in range(0,numberOfLoops):
            ...
            # Verify if we catch all DataPoints
            if ( dataPointNumber != len(datapointList) ):
                testResult = False
                message = "Error verifiying the number of DataPoints sent byt the device"
                self.log.error(message)
                break
    ...
    def verifyDataPointsInDeviceCloud(self, dataStream, datapointList):
        ...
        # Verify if we capture DataPoints
        if ( numberDataPointsUploaded == 0 ):
            self.log.error("Error: No data points were captured sent to Device Cloud!!")
            return False
        ...
        while ( retryCounter < 3 ):
            ...
            if ( numberDataPointsUploaded == len(datapointListFromDeviceCloud) ):
                ...
                for index in range(0,numberDataPointsUploaded):
                    ...
                    #if ( eachDatapointFromDeviceCloud.description != eachDatapointUploaded["description"] ):
                        #message = "DataPoint description does not match!!! readed: '%s' != expected: '%s'\n Original DataPoint: %s\n Readed DataPoint: %s\n" % (eachDatapointFromDeviceCloud.description, eachDatapointUploaded["description"], eachDatapointUploaded, eachDatapointFromDeviceCloud)
                        #self.log.warning(message)
                        #verificationResult = False
                        #break
            else:
    ...
    def readDataPointsFromConsole(self, dataStreamID):

            # Wait until the group of DataPoints are sent
            result, line, readBuffer = self.deviceHandler.readUntilPattern ( pattern="END LOOP ----------------------------------------", timeout=60)

            if ( not result ):
                self.sendErrorAndCleanDataStream("Console feedback for the end of the loop NOT readed.....", dataStreamID);

            # DataPoint structure where save the different elements
            datapointList = []

            patternDatapoint = "^\[DataPoint\]"


            # Find all datapoints from the console
            for index in readBuffer.findAll(patternDatapoint):
                # Initialize vars
                pointLocation = None
                pointQuality = None
    ...
    def sendDeviceRequestToInitiateAction(self, target, payload, expectedResponse="Request successfully processed"):
        ...
        if(status):
            ...
            try:
                ...
                self.logger.error("An error was occurred parsing the response")
                self.logger.exception( "Exception [TypeError]: %s" % (str(e)) )
    ...
    def sendErrorAndCleanDataStream(self, message, dataStreamID):

...
static int remove_registered_target(const char * target) {
	...
	free(req->target);
	elements_to_move = active_requests.size - (req - active_requests.array) - 1; /* count number of valid elements after req */
			   ...
			   ccapi_buffer_info_t *response_buffer_info)
{
	...
	/* the request buffer isn't null terminated, so we create a null terminated string */
...
static ccapi_receive_error_t unregister_target(const char *target)
{
	...
	{
		/* This should never happen, and if it does happen still return OK to the calling process, as the CCAPI did unregister the target */

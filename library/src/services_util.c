...
/*	
	This module encapsulates the local tcp/ip communication between cc_acld and processes
	that use DRM services via the connector.
	
	Messages are composed of a sequence of values that are serialised into the stream and
	deserialised by the receiver.
	...
	The current encoding targets the following goals ...
		...
		-	Easy for a scripted client (eg a Bash script) to compose and parse. No endianess
			considerations, values terminated by new line '\n' characters
		
		-	Serialised value boundaries are verified so that loss of stream synchronisation
			is detected and doesn't cause mis-interpretation of serialised data.
			
		-	The receiver can verify that the data type of any serialised value is exactly
			what was expected rather then relying on some implicit contract between
			sender and receiver.
	...
	Using '\n' as the terminator makes it easy for scripted clients reading line by
	line input as well as Python clients using stream.readline() to parse the stream. 
				
	Having the prefix data length for strings and blobs allows their payload to read
	in a single non-parsed binary read of length+1 then to verify that the last byte
	...
	All the functions required to read/write values from/to socket streams are provided
	by this module.
*/
...
static ssize_t read_line( int socket, void *buffer, size_t capacity, struct timeval *timeout )
{
    ...
    for (;;) {
		...
		if (remaining) {
			...
			/* 	Minimise the number of system calls by reading in largest possible chunks.
				More complex than simply reading char by char but typically only requires
				one pass and two context switches rather than one context switch per char 
				received when reading one char at a time.
			*/
	...
	if (read_amt(fd, rxtype, 2, timeout) == 0					/* Read the type */
		...
		if (buffer) {
			if (read_amt(fd, buffer, length+1, timeout) == 0	/* read the payload + terminator */
				...
				if (data_length) *data_length = length;			/* & report the length to the caller if needed */
				*data = buffer;
				return 0;
			}
			free(buffer);
